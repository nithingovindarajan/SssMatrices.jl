...
function compute_Phi_Psi(V_i, Q_i, D_i, Psi_i, Phi_i, R_iplus1, P_iplus1, W_i, U_i)
    temp =
        Psi_i * W_i - (-Q_i' + Psi_i * V_i') * ((D_i + Phi_i * V_i') \ (Phi_i * W_i + U_i))
...
function forwardsolve(
    ...
    temp =
        r_kmin1 -
        (-Q_kmin1' + Psi_kmin1 * V_kmin1') * ((D_kmin1 + Phi_kmin1 * V_kmin1') \ s_kmin1)
...
function backwarditerate!(x, A, Phi)
    x[A.block[A.no_blocks]], q = backwardsolve(
        A.diagonal.D[A.no_blocks],
        A.upper.out[A.no_blocks],
        Phi[A.no_blocks],
        zeros(A.upper_hankel_ranks[A.no_blocks]),
        x[A.block[A.no_blocks]],
    )
    for k = A.no_blocks:-1:2
        x[A.block[k-1]], q = backwardsolve(
            A.diagonal.D[k-1],
            A.upper.out[k-1],
            Phi[k-1],
            A.upper.trans[k-1]' * q,
            x[A.block[k-1]] - A.upper.inp[k-1] * q,
        )
    end
end
